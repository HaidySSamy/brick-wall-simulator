...
class BrickVisual:
    ...
    def draw(self, surface):
        ...
        if self.ref.built:
            ...
            is_full_stretcher = (
                self.ref.length == 210 and not self.ref.is_half and not self.ref.is_header
            )
            is_english = self.ref.bond_type == "english" if hasattr(self.ref, "bond_type") else False
            needs_pale = (
                is_english and is_full_stretcher and not self.ref.back_to_back
            )

            if needs_pale:
                color = tuple((c + 255) // 2 for c in stride_color)  # pale
            else:
                color = stride_color  # bold
...
def convert_logic_wall_to_visual(wall: LogicWall):
    ...
    for i, row in enumerate(wall.rows):
        ...
        # Apply offset ONLY for Flemish bond, even-numbered rows
        if wall.bond_type == "flemish" and i % 2 == 1:
            x_offset = BRICK_FULL // 4  # ¼-brick = ~52.5 mm
        else:
            x_offset = 0
...
def bond_selection_screen():
    bonds = ["stretcher", "flemish", "english"]
    buttons = []
    margin = 20
    btn_width = 200
    btn_height = 50
